...
public class ThreadPoolExecutor2 extends AbstractExecutorService {
    ...
    final void runWorker(Worker w) {
        ...
        w.firstTask = null;
        w.unlock(); // 允许中断，调用tryRelease()将state状态设置为0(初始化状态时是-1)，当state为0时才允许调用interruptIfStarted()中只有state>=0才允许调用中断
        boolean completedAbruptly = true;
        try {
            //如果传递的worker的firstTask不为空，或者从阻塞队列getTask()不为空
            while (task != null || (task = getTask()) != null) {
                w.lock();//上锁，不是为了防止并发执行任务，将AQS状态设置为1 为了在shutdown()时不终止正在运行的worker
                /*
                 * 线程池当前状态>=STOP(还有TIDYING、TERMINATED状态) || Thread.interrupted()为true，即线程已经被中断，又清除了中断标示，再次判断线程池状态是否>=stop
                 *
                 * 当前线程中断
                 *
                 * if判断是true，再次设置中断标志，wt.interrupt()
                 *
                 *
                 **/
                if (
                ...
                try {
                    ...
                    try {
                        ...
                        //上面的异常一层层的传递下来，最后将精准的异常传递给afterExecute
